...
export class TokenService {
  ...
  async delete(uId: string, token: string): Promise<{ ok?: number, n?: number, deletedCount?: number}> {
    return await this.tokenModel.deleteOne({ uId, token });
  }

  async deleteAll(uId: string): Promise<{ ok?: number, n?: number, deletedCount?: number }> {
    return await this.tokenModel.deleteMany({ uId });
  }

  async exists(uId: string, token: string): Promise<{_id: any}> {
